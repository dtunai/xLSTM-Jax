...
@click.option(
    "--package-name",
    prompt="Python package name (e.g. 'xlstm-jax')",
...
def main(
    ...
    replacements = [
        ...
        ("xlstm-jax", package_actual_name),
        ("xlstm_jax", package_dir_name),
    ]
    if dry_run:
    ...
    # Rename 'xlstm_jax' directory to `package_dir_name`.
    if not dry_run:
        (REPO_BASE / "xlstm_jax").replace(REPO_BASE / package_dir_name)
    else:
        print(f"Renaming 'xlstm_jax' directory to '{package_dir_name}'")
